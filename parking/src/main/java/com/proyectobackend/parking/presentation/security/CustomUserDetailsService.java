package com.proyectobackend.parking.presentation.security;
...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    private Collection<? extends GrantedAuthority> getAuthority(Set<Role> roles) {
        return roles.stream().map(role -> new SimpleGrantedAuthority(role.getName())).collect(java.util.stream.Collectors.toList());
    }
}